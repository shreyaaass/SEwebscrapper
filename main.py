...
def findCourses(search):
    with open(f"{search}.csv",'w',encoding="utf-8") as new: 
        wr=writer(new)
        ...
        #total_count=soup1.find('div',course-card--course-title--vVEjC")
        #print(total_count.text)
        #html_text1=requests.get(f'https://www.udemy.com/courses/search/?p={i}&q={search}&src=ukw').text
findCourses(input("Enter search:\n"))